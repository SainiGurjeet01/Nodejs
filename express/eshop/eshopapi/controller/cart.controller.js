...
export const getCart = async (request,response,next)=>{
    try{
        ...
        let cart = await Cart.findOne({userId}).populate("cartItem.productId");
...
export const deleteCartItem = async (request,response,next)=>{
    ...
    try{
        Cart.updateOne({userId},{
            ...
            console.log(result);
            return result.modifiedCount ? response.status(200)({message:"Item removed from cart"}) : response.status(404).json({error: "Not found"});
        }).catch(err=>{
            console.log(err);
            return response.status(500).json({error: " Internal Server error"});
        })
